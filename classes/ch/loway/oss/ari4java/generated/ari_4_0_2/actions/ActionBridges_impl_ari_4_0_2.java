...
return deserializeJsonAsAbstractList( json,
   new TypeReference<List<Bridge_impl_ari_4_0_2>>() {} ); 
}

@Override
public void list(AriCallback<List<Bridge>> callback) {
buildList();
httpActionAsync(callback, new TypeReference<List<Bridge_impl_ari_4_0_2>>() {});
}
...
String json = httpActionSync();
return deserializeJson( json, Bridge_impl_ari_4_0_2.class ); 
}

@Override
public void create(String type, String bridgeId, String name, AriCallback<Bridge> callback) {
buildCreate(type, bridgeId, name);
httpActionAsync(callback, Bridge_impl_ari_4_0_2.class);
}
 ...
 * Add a channel to a bridge.
 *********************************************************/
private void buildAddChannel(String bridgeId, String channel, String role, boolean absorbDTMF, boolean mute) {
reset();
url = "/bridges/" + bridgeId + "/addChannel";
method = "POST";
lParamQuery.add( HttpParam.build( "channel", channel) );
lParamQuery.add( HttpParam.build( "role", role) );
lParamQuery.add( HttpParam.build( "absorbDTMF", absorbDTMF) );
lParamQuery.add( HttpParam.build( "mute", mute) );
...
buildAddChannel(bridgeId, channel, role, absorbDTMF, mute);
String json = httpActionSync();
}
