...
public class ActionChannels_impl_ari_4_0_2 extends BaseAriAction  implements ActionChannels {
/**********************************************************
 * Active channels
 * 
...
return deserializeJsonAsAbstractList( json,
   new TypeReference<List<Channel_impl_ari_4_0_2>>() {} ); 
}

@Override
public void list(AriCallback<List<Channel>> callback) {
buildList();
httpActionAsync(callback, new TypeReference<List<Channel_impl_ari_4_0_2>>() {});
}
...
lParamQuery.add( HttpParam.build( "formats", formats) );
lE.add( HttpResponse.build( 400, "Invalid parameters for originating a channel.") );
lE.add( HttpResponse.build( 409, "Channel with given unique ID already exists.") );
}
...
String json = httpActionSync();
return deserializeJson( json, Channel_impl_ari_4_0_2.class ); 
}
...
buildOriginate(endpoint, extension, context, priority, label, app, appArgs, callerId, timeout, variables, channelId, otherChannelId, originator, formats);
httpActionAsync(callback, Channel_impl_ari_4_0_2.class);
}
...
lE.add( HttpResponse.build( 412, "Channel in invalid state") );
}
...
method = "POST";
lParamQuery.add( HttpParam.build( "caller", caller) );
lParamQuery.add( HttpParam.build( "timeout", timeout) );
lE.add( HttpResponse.build( 404, "Channel cannot be found.") );
lE.add( HttpResponse.build( 409, "Channel cannot be dialed.") );
}
 ...
 * Move the channel from one Stasis application to another.
 *********************************************************/
private void buildMove(String channelId, String app, String appArgs) {
reset();
url = "/channels/" + channelId + "/move";
method = "POST";
lParamQuery.add( HttpParam.build( "app", app) );
lParamQuery.add( HttpParam.build( "appArgs", appArgs) );
lE.add( HttpResponse.build( 404, "Channel not found") );
...
buildMove(channelId, app, appArgs);
String json = httpActionSync();
}
...
String json = httpActionSync();
return deserializeJson( json, Playback_impl_ari_4_0_2.class ); 
}
...
buildPlay(channelId, media, lang, offsetms, skipms, playbackId);
httpActionAsync(callback, Playback_impl_ari_4_0_2.class);
}
...
String json = httpActionSync();
return deserializeJson( json, LiveRecording_impl_ari_4_0_2.class ); 
}
...
buildRecord(channelId, name, format, maxDurationSeconds, maxSilenceSeconds, ifExists, beep, terminateOn);
httpActionAsync(callback, LiveRecording_impl_ari_4_0_2.class);
}
 ...
 *********************************************************/
private void buildRtpstatistics(String channelId) {
reset();
url = "/channels/" + channelId + "/rtp_statistics";
method = "GET";
lE.add( HttpResponse.build( 404, "Channel cannot be found.") );
}
...
buildRtpstatistics(channelId);
String json = httpActionSync();
return deserializeJson( json, RTPstat_impl_ari_4_0_2.class ); 
}

@Override
public void rtpstatistics(String channelId, AriCallback<RTPstat> callback) {
buildRtpstatistics(channelId);
httpActionAsync(callback, RTPstat_impl_ari_4_0_2.class);
}
...
String json = httpActionSync();
return deserializeJson( json, Variable_impl_ari_4_0_2.class ); 
}

@Override
public void getChannelVar(String channelId, String variable, AriCallback<Variable> callback) {
buildGetChannelVar(channelId, variable);
httpActionAsync(callback, Variable_impl_ari_4_0_2.class);
}
